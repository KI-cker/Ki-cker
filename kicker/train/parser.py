...
class Parser:
    ...
    def get_game_data(self, game):
        data = self.file[game]
        table_frames = [self.decode_image(i)
                        for i in data['table_frames_encoded']]
    ...
    def get_train_game_data(self, game):
        data = self.file[game]
        table_frames = [self.decode_image(i)[:, :, 1]
                        for i in data['table_frames_encoded']]
        positions = [p for p in data['ball_pos']]
        actions = [a for a in data['actions']]
        ...
        observations = [np.swapaxes(np.swapaxes(
            table_frames[k:k + 6], 0, 2), 0, 1) for k in range(0, length - 7)]
