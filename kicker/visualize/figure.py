...
class Figure():
    ...
    def build_plot_with_images(self):
        ...
        self.inner = gridspec.GridSpecFromSubplotSpec(
            1, 3, subplot_spec=self.outer[0])
        self.inner_plots = gridspec.GridSpecFromSubplotSpec(
            1, self.degrees_of_freedom, subplot_spec=self.outer[1])
        self.history_plots = gridspec.GridSpecFromSubplotSpec(
            1, int(self.degrees_of_freedom / 2), subplot_spec=self.outer[2])

    def plot(self, before, now, after, prediction):
        self.figure.clf()

        if self.show_images:
            axes = [pyplot.Subplot(self.figure, self.inner[j])
                    for j in range(3)]
        ...
        for j in range(self.degrees_of_freedom):
            ...
            sns.barplot(y=prediction[j, :] - np.min(prediction[j, :]),
                        x=['Backward', 'Null', 'Forward'], ax=axes)
    ...
    def build_plot_without_images(self):
        ...
        self.inner_plots = gridspec.GridSpecFromSubplotSpec(
            1, self.degrees_of_freedom, subplot_spec=self.outer[0])
        self.history_plots = gridspec.GridSpecFromSubplotSpec(
            1, self.degrees_of_freedom, subplot_spec=self.outer[1])
